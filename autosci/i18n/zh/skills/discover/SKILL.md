...
- `wiki/sources/*.md` —— frontmatter 中的 `arxiv`（或旧版 `arxiv_id`），用于与已 ingest 的论文做 dedup
- `wiki/sources/*.md` 修改时间 —— 用于 `--from-wiki` 模式下 anchor 选取
- `wiki/sources/*.md`、`wiki/concepts/*.md`、`wiki/topics/*.md` —— 标题与正文，用于 venue 模式的相关性打分
...
- **`--from-wiki` 找不到可用 anchor**（`wiki/sources/` 为空或全部缺少 `arxiv_id`）：告诉用户 wiki 过于稀疏，建议改用 topic 模式（或跑 `/init`）。
